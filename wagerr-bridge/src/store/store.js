...
const endpoints = {
  ...
  getUncomfirmedWagerrTransactions: '/api/v1/getUncomfirmedWagerrTransactions',
...
class Store extends EventEmitter {
    ...
    dispatcher.register(async payload => {
      switch(payload.type) {
        case Actions.GET_INFO:
          this.getInfo();
          break;
        case Actions.GET_UNCONFIRMED_WAGERR_TXS:
          this.getUnconfirmedWagerrTransactions(payload);
          break;
  ...
  async getUnconfirmedWagerrTransactions(payload) {
    try {
      const data = await this.fetch(endpoints.getUncomfirmedWagerrTransactions, 'GET', payload.content);
      this.emit(Events.FETCHED_UNCONFIRMED_WAGERR_TXS, data.result);
    } catch (e) {
      this.emit(Events.ERROR, e);
    }
  }
